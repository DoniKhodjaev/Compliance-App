...
export function SDNList() {
  ...
  const fetchSDNList = async () => {
    try {
      const response = await axios.get(
        import.meta.env.VITE_BACKEND_URL1 + "/api/sdn-list",
      );
  ...
  const updateSDNList = () => {
    setIsUpdating(true);
    axios
      .post(import.meta.env.VITE_BACKEND_URL1 + "/api/update-sdn-list")
      .then((response) => {
  ...
  const enhancedMatch = (name: string, query: string) => {
    ...
    return queryTokens.every((queryToken) =>
      nameTokens.some(
        (nameToken) =>
          stringSimilarity.compareTwoStrings(nameToken, queryToken) >=
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {currentEntries.length > 0 ? (
              ...
              <tr>
                <td
                  colSpan={5}
                  className="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500 dark:text-gray-300"
                >
                  No entries found
                </td>
              </tr>
